...
minifiedCode = open('main.c1506a22.js', mode="r", encoding="utf-8").read()
...
def getNextWord(previousAttempts):
  ...
  for word in candidates:
    scoreParts = {}
    for i in range(5):
      if word[i] not in scoreParts:
        scoreParts[word[i]] = 0
      scoreParts[word[i]] = max(scoreParts[word[i]], stats[i][word[i]])
      
    score = sum(scoreParts.values())
    if score > maxScore:
      maxScore = score
      bestWord = word
  return bestWord
